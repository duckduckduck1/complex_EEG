...
FlutterBluePlusWinrtPlugin::~FlutterBluePlusWinrtPlugin() {
    is_alive_ = false;
    StopDeviceWatcher(); // FORK (complex_EEG)
...
void FlutterBluePlusWinrtPlugin::OnAdvertisementStopped(
    ...
    // FORK (complex_EEG): the upstream handler is empty, so a watcher that
    // Windows aborts (radio contention, driver error) dies silently while the
    // app keeps "scanning". At least leave a trace in the debug log.
    Log("AdvertisementWatcher stopped");
}

// FORK (complex_EEG): Windows schedules a bare BluetoothLEAdvertisementWatcher
// lazily — the radio listens only a small fraction of time, so the first
// sighting of a device can take many seconds while system scanners (Bluetooth
// LE Explorer) see it almost instantly. The fix is to run an AEP DeviceWatcher
// in parallel, EXACTLY as microsoft/BluetoothLEExplorer does — the point is
// not the enumeration results (we ignore them) but that requesting the live
// SignalStrength/IsPresent/LastSeenTime properties forces Windows into
// *continuous active* scanning. Two things matter and both were wrong in the
// first attempt: (1) the correct BLE AEP ProtocolId, and (2) a non-empty
// requestedProperties list — without it the watcher does a one-shot
// enumeration and does not keep the radio scanning.
void FlutterBluePlusWinrtPlugin::StartDeviceWatcher() {
    try {
        StopDeviceWatcher();
        // BLE Explorer's exact AQS + protocol id (Bluetooth LE AEP).
        winrt::hstring aqs =
            L"(System.Devices.Aep.ProtocolId:=\"{bb7bb05e-5972-42b5-94fc-76eaa7084d49}\")";
        // The live properties whose tracking forces continuous active scan.
        std::vector<winrt::hstring> requested_properties{
        ...
        // Handlers must be attached before Start(); bodies are intentionally
        // empty (results unused). A fresh watcher instance is created per
        // startScan to avoid Start-while-Stopping state errors on quick
        // stop/start cycles.
        device_watcher_ = DeviceInformation::CreateWatcher(
            aqs, requested_properties, DeviceInformationKind::AssociationEndpoint);
        device_watcher_.Added([](DeviceWatcher const&, DeviceInformation const&) {});
        device_watcher_.Updated([](DeviceWatcher const&, DeviceInformationUpdate const&) {});
        device_watcher_.Removed([](DeviceWatcher const&, DeviceInformationUpdate const&) {});
        device_watcher_.EnumerationCompleted([](DeviceWatcher const&, IInspectable const&) {});
        device_watcher_.Stopped([](DeviceWatcher const&, IInspectable const&) {});
        device_watcher_.Start();
        Log("DeviceWatcher started (active scan, BLE Explorer recipe)");
    } catch (...) {
        // Non-fatal: the advertisement watcher still works, just lazily.
        Log("DeviceWatcher start failed");
        device_watcher_ = nullptr;
    }
}

void FlutterBluePlusWinrtPlugin::StopDeviceWatcher() {
    if (device_watcher_) {
        try { device_watcher_.Stop(); } catch (...) {}
        device_watcher_ = nullptr;
    }
}
...
void FlutterBluePlusWinrtPlugin::HandleMethodCall(const flutter::MethodCall<flutter::EncodableValue>& method_call, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    ...
    if (method == "startScan") { scan_results_cache_.clear(); watcher_.Start(); StartDeviceWatcher(); result->Success(flutter::EncodableValue(true)); return; }
    if (method == "stopScan") { StopDeviceWatcher(); watcher_.Stop(); result->Success(flutter::EncodableValue(true)); return; }
