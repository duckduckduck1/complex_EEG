...
class FlutterBluePlusWinrtPlugin : public flutter::Plugin {
 ...
 private:
  ...
  // FORK (complex_EEG): parallel DeviceWatcher running only while scanning.
  // A bare BluetoothLEAdvertisementWatcher is scheduled lazily by Windows
  // (low scan duty cycle) and first sighting of a device can take many
  // seconds; an active AEP DeviceWatcher makes Windows scan aggressively.
  // See third_party/flutter_blue_plus_winrt/README_FORK.md.
  ...
  // FORK (complex_EEG): lifecycle of the aggressive-scan DeviceWatcher.
