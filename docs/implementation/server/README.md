...
upload.md       - загрузочные сессии и приём папок экспериментов
...
- Загрузка файлов: upload session + multipart file upload.
...
принципы: запись ЭЭГ остаётся локальной, сервер принимает только завершённые
эксперименты, исходные файлы не переписываются.
