# Запись сигнала

Документ описывает реализацию записи эксперимента: старт/стоп, буфер, запись на
диск, сегменты, события ФБМ и качество данных.

---

## BLoC

```text
RecordingBloc
```

Весь жизненный цикл записи ведёт один `RecordingBloc`: сегменты, разрывы,
фильтры, ФБМ и метки. Отдельных BLoC под сегменты, качество или ФБМ нет —
они завязаны на один и тот же счётчик отсчётов, и дробление добавляло бы
синхронизацию без выгоды.

---

## Жизненный цикл записи

```text
idle
preparing
recording
paused_by_disconnect
stopping
stopped
failed
recovering
```

Разрешённые переходы:

```text
idle -> preparing -> recording
recording -> paused_by_disconnect
paused_by_disconnect -> recording
recording -> stopping -> stopped
paused_by_disconnect -> stopping -> stopped
any -> failed
failed -> recovering
recovering -> recording | stopped
```

---

## Старт записи

Последовательность старта:

1. пользователь заполняет форму метаданных;
2. приложение генерирует `experiment_id = exp_<ULID>`;
3. создаёт папку эксперимента;
4. создаёт пустой `signal.bin`;
5. создаёт `journal.ndjson`;
6. проектирует цепочку потоковых фильтров записи по выбранным настройкам
   (`lp`/`hp`/`notch`);
7. пишет событие `experiment_started`;
8. открывает первый сегмент;
9. переводит состояние в `recording`.

Если любой шаг падает, запись не считается начатой.

---

## Запись сигнала на диск

`signal.bin` содержит последовательность **фильтрованных** значений амплитуды
`int32` little-endian в мкВ. Исходный raw-поток BLE в MVP не сохраняется.
Применённые фильтры записываются в `experiment.json` как
`recording.filters`.

Политика записи:

```text
append-only
интервал сброса: 10 секунд
настраивается
```

Отсчёты сигнала:

1. приходят из BLE-декодера;
2. проходят через stateful streaming filter, созданный один раз на старте
   записи;
3. помещаются в буфер записи в памяти;
4. каждые 10 секунд пачка пишется в `signal.bin`;
5. график читает из буфера в памяти, не с диска.

При штатной остановке выполняется финальный сброс буфера на диск.

Фильтр записи не переиспользует display-фильтр графика, который
перефильтровывает весь буфер для визуализации. Для записи каждый отсчёт
обрабатывается потоково, без пересчёта прошлого окна.

---

## Запись журнала

`journal.ndjson` пишется в режиме append-only.

Критичные события синхронизируются немедленно:

```text
experiment_started
segment_started
segment_ended
connection_lost
connection_resumed
recording_stopped
fbm_event
annotation_created
annotation_updated
annotation_deleted
```

Критичные граничные события выполняют принудительный сброс на диск (flush/sync).

---

## Сегменты

Сегмент — непрерывный участок сигнала между обрывами BLE.

Поля сегмента:

```text
segment_id
start_sample
end_sample
started_at_wall_clock
ended_at_wall_clock optional
```

`segment_id` генерируется приложением как `seg_<N>`, где `N` — порядковый номер
сегмента внутри эксперимента, начиная с 1.

Диапазон:

```text
[start_sample, end_sample)
```

`start_sample` и `end_sample` — глобальные индексы отсчётов в `signal.bin`.

Разметка использует:

```text
segment_id + segment_sample_index
```

---

## Обработка обрыва связи

При обрыве BLE:

1. буфер сбрасывается на диск;
2. текущий сегмент закрывается на границе последнего сохранённого/буферизованного
   отсчёта;
3. событие `connection_lost` пишется в журнал;
4. состояние переходит в `paused_by_disconnect`;
5. команда ФБМ запрещается;
6. график показывает явный разрыв.

Сигнал за время обрыва отсутствует и никогда не достраивается: в `signal.bin` не
пишутся синтетические нули, а разрыв отражается через закрытый сегмент и запись
в `gaps`.

---

## Ручное переподключение

Переподключение инициирует пользователь.

После переподключения:

1. открывается новый сегмент;
2. приблизительная длина разрыва вычисляется по настенным часам;
3. события `connection_resumed` и `segment_started` пишутся в журнал;
4. отсчёты дописываются в тот же `signal.bin`.

---

## Фотобиомодуляция

Команда ФБМ доступна только в состоянии `recording`.

При вызове:

1. `RecordingBloc` проверяет статус записи;
2. отправляется команда по BLE;
3. событие немедленно пишется в `journal.ndjson`;
4. UI показывает результат команды.

Уровень ШИМ задаётся в диапазоне `1..99` и переводится в байт командой
`round(level * 255 / 100)`. Если ФБМ выключена, изменение уровня только меняет
настройку следующего включения и не создаёт `fbm_event`.

Поля события:

```text
segment_id
sample_index
segment_sample_index
global_sample_index
wall_clock_time
on
pwm_level
pwm_byte
command_delivered
reason optional
```

`sample_index` и `global_sample_index` — глобальные индексы в `signal.bin`;
`segment_sample_index` — локальный индекс внутри текущего сегмента. При штатном
stop приложение сначала отправляет off-команду ФБМ и пишет событие с
`reason: recording_stop`. При физическом обрыве BLE команда может быть уже
недоставима; тогда событие авто-выключения пишется с `command_delivered: false`
и `reason: connection_lost`, чтобы журнал не выдавал попытку за доставленную
команду.

---

## Контроль качества

Автоматических индикаторов качества (клиппинг, flatline, диапазон амплитуды)
сейчас нет. Оператор оценивает сигнал по живым графикам, а плохой участок
помечает вручную как брак — это событие разметки, `signal.bin` оно не меняет
(см. [annotation.md](annotation.md)).

---

## Размер файлов

Явного лимита на размер `signal.bin` приложение не задаёт. При 250 отсчётах в
секунду и `int32` файл растёт примерно на 1 КБ/с — около 3.6 МБ в час, поэтому
многочасовой эксперимент остаётся небольшим.

Контроля свободного места на диске сейчас нет.

---

## Остановка записи

Последовательность остановки:

1. перестать принимать новые отсчёты;
2. финальный сброс буфера сигнала;
3. закрыть открытую метку-состояние, если она есть;
4. закрыть активный сегмент;
5. записать `recording_stopped`;
6. собрать финальный `experiment.json`;
7. перевести состояние в `stopped`.

Финальный `experiment.json` содержит `recording.filters`, список закрытых
сегментов, `gaps`, `labels` и `fbm_events`. В `labels` попадают только закрытые
метки с глобальными координатами `sample_index` или `start_sample`/`end_sample`
и дополнительными локальными координатами для UI. Raw-поток не
восстанавливается из фильтрованного файла и не прикладывается отдельным файлом.

---

## Проверки реализации

- фильтрованные отсчёты пишутся как `int32` little-endian;
- применённые фильтры отражены в `experiment.json`;
- буфер сбрасывается каждый настроенный интервал;
- журнал пишет критичные события немедленно;
- обрыв закрывает сегмент;
- обрыв закрывает открытую метку-состояние на конце сегмента;
- переподключение открывает новый сегмент;
- при обрыве в `signal.bin` нет синтетических нулей;
- ФБМ запрещена в `paused_by_disconnect`;
- черновые метки не попадают в финальный `experiment.json`;
- остановка создаёт финальный `experiment.json`;
- два устройства пишут две независимые папки экспериментов.
