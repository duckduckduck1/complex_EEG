...
- Сервер не в критическом пути записи: падение сервера или потеря сети не мешают
  локальному сбору данных в приложении.
- Принимает только завершённые эксперименты через Web UI; загрузке предшествует
  аутентификация. Прямая загрузка из Flutter в MVP не входит.
- Данные не исправляются молча: некорректный пакет → понятная ошибка валидации.
- Исходный пакет (`signal.bin`, `experiment.json`) неизменяем; обработка создаёт
  только производные результаты.
- Каждый этап имеет статус (uploading → uploaded → validating → accepted →
  processing → processed, плюс статусы ошибок).
- Не входит: live-streaming сигнала, управление BLE-устройством и ФБМ, локальный
  recovery записи, ручная разметка, авто-определение фаз сна, ML/MLOps.
