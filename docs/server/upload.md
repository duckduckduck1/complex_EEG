# Загрузка эксперимента (upload flow)
...
папку эксперимента или отдельные файлы пакета. Web UI отправляет каждый
выбранный пакет как независимый upload flow. Сервер принимает только завершённый
пакет эксперимента.

`.zip archive` пока остаётся dev/test bridge или будущим production flow:
production Web UI не вызывает dev/test endpoint'ы.
