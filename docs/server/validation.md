...
- Валидация стоит до принятия: пока проверка не пройдена, эксперимент не
  `accepted`.
- Исходные данные не изменяются: валидатор только читает и проверяет структуру.
- Ошибки конкретны и понятны пользователю (машиночитаемые коды — см.
  [reference/experiment_package](../reference/experiment_package.md)).
- Проверка защищает следующие этапы: пайплайн и веб не получают битый пакет.
- Не делает: автоисправление JSON, правку разметки, склейку сегментов,
  заполнение разрывов, фильтрацию сигнала, запуск обработки, научную оценку
  качества.
