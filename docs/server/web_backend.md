...
- Веб работает с серверными данными: показывает уже загруженное/принятое и
  статусы.
- Web UI — основной upload-клиент MVP: пользователь выбирает папку/архив, UI
  создаёт upload session и шлёт файлы.
- Исходные данные через веб не редактируются; запись и разметка — функции
  Flutter-приложения.
- Доступ только после аутентификации; статусы видимы; ошибки читаемы (причина, а
  не абстрактный сбой).
- Не входит: live-просмотр сигнала, управление BLE/ФБМ, правка бинарника,
  авторазметка, авто-определение фаз сна, ML, сложная ролевая модель.
