...
- Мониторинг отвечает на вопрос «система здорова или нет»: состояние сервисов,
  данных и фоновых задач.
- Алерты только на реальные проблемы; шумный мониторинг бесполезен.
- Бэкапы тоже мониторятся (важно знать, что бэкап действительно выполнился).
- Метрики дополняют логи: метрики показывают состояние, логи — причину.
- Не входит: автоисправление без решения оператора, замена runbook, хранение
  исходных данных, анализ научного качества ЭЭГ, авторазметка, ML/MLOps.
