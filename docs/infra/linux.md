...
- Сервер настраивается не как личный ноутбук: только нужные пользователи,
  сервисы, директории и порты.
- Доступ контролируемый (через SSH, без лишних открытых сервисов).
- Данные (эксперименты, PostgreSQL volume, бэкапы, логи) отделены от приложения
  и живут в понятных директориях вне контейнеров.
- Ручные действия, нужные для работы сервера, потом переносятся в Ansible.
- Не входит: приём/валидация/обработка ЭЭГ, веб и бизнес-логика, секреты в
  открытом виде, ручная настройка без переноса в Ansible.
