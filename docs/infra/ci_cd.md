# CI/CD — доставка изменений

## Принципы и границы

- Изменения не попадают на сервер без проверок (тесты, линтеры, сборка образов,
  валидность конфигурации).
- Деплой повторяемый; откат к предыдущей рабочей версии возможен.
- Данные не зависят от деплоя: обновление кода/контейнера не трогает
  эксперименты, PostgreSQL volume и бэкапы.
- Миграции БД требуют особого внимания и совместимости с бэкапами.
- Не входит: сложный release train, blue-green/canary, Kubernetes delivery,
  автодеплой в production без подтверждения, деплой данных экспериментов.
