...
- Контейнеры одноразовые, данные постоянные (volume / хостовая ФС).
- У каждого сервиса одна ответственность; API, БД, пайплайн, web, мониторинг не
  смешиваются в одном контейнере.
- Конфигурация передаётся явно (env, compose, Ansible), а не правится внутри
  контейнера; секреты не попадают в образ.
- Не входит: хранение единственной копии данных в контейнере, всё в одном
  контейнере, секреты в образе, Kubernetes на текущем этапе, бизнес-логика ЭЭГ.
