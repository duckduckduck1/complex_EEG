...
```text
notify BLE-характеристики (BleConnection.packets)
  -> BleSampleDecoder            3 байта -> микровольты
  -> RecordingBridge             -> RecordingBloc  -> signal.bin
  -> DeviceEegBridge             -> RtEegDataBloc  -> графики
```

Один поток пакетов расходится к двум независимым потребителям через мосты:
запись и живые графики не мешают друг другу.
...
сигнала: порт `BleConnection` даёт для этого `writeCommand(List<int> frame)`.
...
записи: `RecordingBloc` вызывает команду через порт `FbmTransport` и фиксирует
событие в `journal.ndjson` (и далее в `fbm_events` пакета). Так ФБМ всегда
привязана к активному эксперименту и попадает в его источник истины — а вне
записи не работает вовсе.
